...
fn compute_roll<RNG: Rng>(mut dice: Pairs<Rule>, rng: &mut Option<&mut RNG>) -> Result<SingleRollResult> {
...
pub(crate) fn compute<RNG: Rng>(expr: Pairs<Rule>, rng: &mut Option<&mut RNG>) -> Result<SingleRollResult> {
    get_climber().climb(
        expr,
...
pub(crate) fn roll_dice<RNG: Rng>(num: u64, sides: u64, rng: &mut Option<&mut RNG>) -> Vec<u64> {
    if let Some(rng) = rng {
